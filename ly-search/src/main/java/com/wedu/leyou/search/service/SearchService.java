package com.wedu.leyou.search.service;
...
import com.wedu.leyou.search.client.BrandClient;
import com.wedu.leyou.search.client.CategoryClient;
import com.wedu.leyou.search.client.GoodsClient;
import com.wedu.leyou.search.client.SpecificationClient;
import com.wedu.leyou.search.pojo.Goods;
import com.wedu.leyou.search.pojo.SearchRequest;
import com.wedu.leyou.search.pojo.SearchResult;
import com.wedu.leyou.search.repository.GoodsRepository;
...
public class SearchService {
    ...
    public Goods bulidGoods(Spu spu) {
        ...
        //查询规格参数
        List<SpecParam> params = specClient.queryParamList(null, spu.getCid3(),null, true);
        ...
        String specifications = spuDetail.getSpecialSpec().substring(1,spuDetail.getSpecialSpec().length() - 2);
        //获取通用规格参数
        Map<String, Object> genericSpec = JsonUtils
                .nativeRead(specifications, new TypeReference<Map<String,Object>>(){});
        //获取特有规格参数
        Map<String, List<Object>> specialSpec = JsonUtils
                .nativeRead(spuDetail.getGenericSpec(), new TypeReference<Map<String, List<Object>>>() {});
        ...
        for (SpecParam param : params) {
            ...
            //判断是否是通用规格
            if(param.getGeneric()){
                value = genericSpec.get(param.getGroupId());
                if(param.getNumeric()) {
                    //处理成段
                    chooseSegment((String) value, param);
                }
            }else{
        ...
        goods.setSkus(JsonUtils.serialize(skus)); // 所有sku的集合json格式
        goods.setSpecs(spec); // 所有的可搜索的规格参数
    ...
    private List<Map<String,Object>> buildSpecificationAgg(Long cid, QueryBuilder basicQuery) {
        ...
        //1、查询需要聚合的规格参数
        List<SpecParam> params = specClient.queryParamList(null, cid, null,true);
        // 2、聚合
        NativeSearchQueryBuilder queryBuilder = new NativeSearchQueryBuilder();
        // 2.1、带上查询条件
        queryBuilder.withQuery(basicQuery);
    ...
    public void deteleIndex(Long spuId) {
